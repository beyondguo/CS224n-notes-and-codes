...
def minibatch_parse(sentences, model, batch_size):
    ...
    """
    # 这个函数到底想干嘛：
    一个model可以同时对多个partial_parses进行预测得到transitions，所以考虑使用batch处理。
    而每一个句子，需要多次parse才能处理完；
    维护一个“没处理完的parse”数组，不断地依次进行parse_step，遇到处理完的，就把其移除，直到这个数组空了，
    这样，该batch中的所有句子都被parse完成。
    而我们不用单独每一次记录一下获得的dependency，因为partial_parse都是对象，每次parse_step都自动更新了其dependency，我们只用查询其属性值即可。
